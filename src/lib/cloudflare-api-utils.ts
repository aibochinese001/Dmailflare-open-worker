...
export function getEmailWorkerName(env?: CloudflareEnv): string {
	// Allows deployments that renamed the Worker (e.g. mailflare-opc) to keep
	// Email Routing rules pointing at the actual deployed Worker name.
	return env?.CF_EMAIL_WORKER_NAME || "mailflare";
}
